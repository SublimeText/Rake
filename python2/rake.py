...
def createOrGetOutput(window):
    if "create_output_panel" in dir(window):
        return window.create_output_panel("exec")
    else:
        return window.get_output_panel("exec")
...
class RakeCommand(sublime_plugin.WindowCommand, ProcessListener):
    ...
    def run(self, tasks=[], options=[], prefix=[],
        ...
        if not hasattr(self, 'output_view'):
            ...
            self.output_view = createOrGetOutput(self.window)
